...
def read_prb_hatlab(file):
    ...
    for i, group in prb["channel_groups"].items():
        ...
        probe.set_contacts(
            positions=positions, shapes="circle", shape_params={"radius": prb['radius']}, shank_ids=chans,
    ...
    while not opened:
        try:
            with NWBHDF5IO(nwbfile_path, 'r+') as io:
                ...
                for frame_times, event_info, exp_name, kfold in zip(saveData['frameTimes_byEvent'],
                                                                    saveData['event_info'], saveData['experiments'],
                                                                    kin_folders):
                    ...
                    if video_events_intervals_name in nwbfile.intervals.keys():
                        ...
                        video_events.add_column(name="video_session",
                                                description="video session number of recorded video files")
                        video_events.add_column(name="analog_signals_cut_at_end",
                                                description="The number of analog signals (if any) that occurred after the end of video recording session. If they existed, they were cut during processing.")
                    ...
                    for eventIdx, timestamps in enumerate(frame_times):
                        if event_info.ephys_session[eventIdx] == sessNum:
                            series_name = '%s_s_%d_e_%s_timestamps' % (event_info.exp_name[eventIdx],
                                                                       ...
                                                                       str(int(event_info.episode_num[eventIdx])).zfill(
                                                                           3))
                            ...
                            if series_name not in nwbfile.processing[timestamps_module_name].data_interfaces.keys():
                                ...
                                if not epi_mod_already_exists or not any(
                                        video_events_df.start_time == event_info.start_time[eventIdx]):
                                    video_events.add_row(start_time=event_info.start_time[eventIdx],
                                                         ...
                                                         analog_signals_cut_at_end=
                                                         event_info.analog_signals_cut_at_end_of_session[eventIdx])
